...
function Get-Gpus {
  $out = @()
  $nvidiaDone = $false
  $overallUsage = Get-GpuOverallUsagePercent
  ...
  Get-CimInstance Win32_VideoController | ForEach-Object {
    ...
    try {
      ...
      if($raw -gt 0){ $dedicatedMb = Round2($raw / 1MB) }
    } catch {}
    ...
    $sharedMb = $null
    if($isIntegrated -and $sharedLimitMb){ $sharedMb = $sharedLimitMb }

    $usage = $null
    if($overallUsage -ne $null){ $usage = $overallUsage }

    $out += [ordered]@{
      name=$name
      memory_total_mb=$dedicatedMb
