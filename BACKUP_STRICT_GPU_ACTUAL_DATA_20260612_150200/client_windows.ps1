...
function Get-Gpus {
  ...
  $sharedLimitMb = $null
  if($totalRamMb){ $sharedLimitMb = Round2($totalRamMb / 2) }
  ...
  Get-CimInstance Win32_VideoController | ForEach-Object {
    ...
    $sharedMb = $null
    $displayTotalMb = $adapterMb

    if($isIntegrated -and $sharedLimitMb){
      # Task Manager shows integrated GPU memory mostly as shared system memory.
      # Win32_VideoController.AdapterRAM often reports only 128MB/1GB/2GB and is misleading.
      $sharedMb = $sharedLimitMb
      $displayTotalMb = $sharedMb
    }

    $out += [ordered]@{
      name=$name
      memory_total_mb=$displayTotalMb
      dedicated_memory_mb=$adapterMb
