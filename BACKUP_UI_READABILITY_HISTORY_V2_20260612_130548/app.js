...
function roleLabel(){return `${state.username||'user'} â€¢ ${state.role||'viewer'}`;}
...
function machineLabel(m){return `${host(m)} â€¢ ${m.primary_ip||((m.all_ips||[])[0]||'No IP')}`;}
...
function renderDashboard(){
  ...
  $('#kHealthNote').textContent = `${isp} â€¢ live server probe â€¢ clients every 5 sec â€¢ offline after about ${OFFLINE_EXPECTED_SECONDS} sec`;
  ...
  $('#topUsage').innerHTML = [...state.machines].sort((a,b)=>(Number(b.ram_percent||0)+Number(b.cpu_percent||0))-(Number(a.ram_percent||0)+Number(a.cpu_percent||0))).slice(0,5).map(m=>`<div class="usage-row"><div><strong>${esc(host(m))}</strong><small>${esc(m.primary_ip||'No IP')} â€¢ ${esc(m.os||'')}</small></div><div class="usage-mini"><span>CPU ${fmt(m.cpu_percent,'%')} Â· RAM ${fmt(m.ram_percent,'%')} Â· Disk ${fmt(m.disk_max_percent,'%')}</span><div class="bar"><i style="width:${Math.min(100,Number(m.disk_max_percent||0))}%"></i></div></div></div>`).join('') || '<div class="empty">No machine data yet.</div>';
  const latest=(o.changes||[]).slice(0,5); $('#latestChanges').innerHTML = latest.map(ch=>`<div class="change-mini"><strong>${esc(ch.human_title||ch.title||'Change')}</strong><small>${esc(ch.hostname||'')} â€¢ ${ago(ch.created_at)}</small><span>${esc(ch.human_message||ch.message||'')}</span></div>`).join('') || '<div class="empty">No changes yet.</div>';
  $('#latestAlerts').innerHTML=(o.notifications||[]).map(a=>`<div class="item"><div><strong>${esc(a.title)}</strong><small>${esc(a.hostname||'Server')} â€¢ ${ago(a.created_at)}</small><div>${esc(a.message||'')}</div></div><span class="pill ${esc(a.severity||'info')}">${esc(a.severity||'info')}</span></div>`).join('') || '<div class="empty">No alerts yet. Go to Notifications and press Send Test to verify delivery.</div>';
...
function renderCommandSystemSpotlight(m){
  ...
  el.innerHTML=`<div class="spot-head"><div><span class="eyebrow">Selected System Analytics</span><h2>${esc(host(m))}</h2><p>${esc(m.primary_ip||'No IP')} â€¢ ${esc(m.os||'')}</p></div>${statusPill(m)}</div><div class="ring-row">${ring('CPU',m.cpu_percent)}${ring('RAM',m.ram_percent)}${ring('Disk',m.disk_max_percent)}</div><div class="spot-kv"><div><span>Download now</span><strong>${fmt(m.wan_download_mbps,' Mbps',2)}</strong></div><div><span>Upload now</span><strong>${fmt(m.wan_upload_mbps,' Mbps',2)}</strong></div><div><span>Today data</span><strong>â†“ ${fmt(m.today_download_gb,' GB',2)} / â†‘ ${fmt(m.today_upload_gb,' GB',2)}</strong></div><div><span>Inventory</span><strong>${esc(m.usb_count||0)} USB â€¢ ${esc(m.software_count||0)} apps</strong></div></div><div class="spot-actions"><button class="btn small" onclick="switchPage('machine360')">Open 360</button><button class="btn small download-only" onclick="downloadCurrentMachine()">Download selected CSV</button></div>`;
}
