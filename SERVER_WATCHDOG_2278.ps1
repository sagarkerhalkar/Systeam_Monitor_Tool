...
while ($true) {
  try {
    ...
    Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -match "SagarSystemHealthMonitor.*server\.py" } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }
    $python = Find-Python
    if (-not $python) { Log "Python not found"; Start-Sleep 30; continue }
    Log "Starting server with $python"
    Start-Process -FilePath $python -ArgumentList @("-u",(Join-Path $App "server.py"),"--host","0.0.0.0","--port","2278") -WorkingDirectory $App -WindowStyle Hidden -RedirectStandardOutput $ConsoleLog -RedirectStandardError $ErrorLog | Out-Null
    Start-Sleep 8
