            ...
            if filename.endswith(".log") and filename != "wan_speed.json":
                ...
                with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
                    try:
                        ...
                        # CROSS-PLATFORM STORAGE PARSING (FOR WINDOWS & UBUNTU LINUX)
                        ssd_p = float(data.get('ssd_used_pct', 0.0))
                        hdd_p = float(data.get('hdd_used_pct', 0.0))
                        
                        # Fallback calculation if node is a Linux endpoint reporting raw arrays
                        ...
                        # FIXED ACCUMULATOR INTERFACE LOOKUP FOR MISLEADING PACKETS
