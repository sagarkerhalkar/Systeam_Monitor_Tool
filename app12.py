...
# ================= FIXED: UNIVERSAL HARDWARE USB TRANSLATOR ENGINE =================
def translate_hardware_peripherals(raw_peripherals_list):
    """
    Cleans up peripheral blocks dynamically. If it encounters raw Linux vendor 
    hex IDs, it matches them up with friendly system hardware names automatically.
    """
    ...
    # Universal look-up map for common hardware vendor IDs across your motherboard architectures
    hardware_lookup_map = {
    ...
    for item in raw_peripherals_list:
        # If the payload is arriving as a clean dictionary from Windows clients
        if isinstance(item, dict):
            name = item.get("Name", "Generic System Device").strip()
            if name:
                cleaned_list.append({"Connected Peripheral": name})
                
        # If the payload is arriving as a raw hex string from Linux clients
        elif isinstance(item, str):
            # Check if it's a raw space-separated string chain (e.g. "ID 1d6b ID 2357")
            if "ID" in item or "id" in item:
                ...
                for idx, word in enumerate(words):
                    if word.upper() == "ID" and idx + 1 < len(words):
                        ...
                        # If we have a match in our engineering database dictionary
    ...
    # Safeguard to prevent empty dataframes from displaying broken boxes
    if not cleaned_list:
...
def render_dashboard_metrics():
    ...
    if rows:
        ...
        if target_pc:
            ...
            # FIXED: Runs raw lists through the look-up mapping function cleanly before creating the UI frame
