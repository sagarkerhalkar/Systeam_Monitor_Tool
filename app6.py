...
def generate_inventory_csv(target_date_str):
    ...
    try:
        ...
        for host, info in devices.items():
            # Cleanly consolidate complex disk details into a readable list cell
            ...
            # Extract peripheral items safely
            periph_list = info.get("peripherals", [])
            periph_str = ", ".join([p.get("Name", "Device") for p in periph_list]) if periph_list else "None"

            export_list.append({
                "Computer Name": host,
                "Status": "Online" if (time.time() - info.get("last_seen_timestamp", 0)) <= 240 else "Offline",
                "Processor (CPU)": info.get("cpu_name", "N/A"),
                "CPU Serial ID": info.get("cpu_serial", "N/A"),
                "Installed RAM (GB)": f"{info.get('ram_total_gb', 0)} GB",
                "Primary GPU Hardware": info.get("gpu_primary", "Integrated Graphics"), # FIXED: Maps correctly to gpu_primary payload field
...
if csv_file:
    ...
    with col2:
        st.download_button(
            label=f"💾 Download Complete Inventory Report (.CSV)",
            data=csv_file,
            file_name=f"Next_Toppers_Detailed_Inventory_{date_str}.csv",
            mime="text/csv",
        )
else:
