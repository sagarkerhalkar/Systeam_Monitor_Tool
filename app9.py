...
def render_dashboard_metrics():
                ...
                with open(filepath, "r") as f:
                    try:
                        ...
                        rows.append({
                            ...
                            "HDD Used %": hdd_p,
                            "GPU Name": gpu_name,
                            "GPU VRAM": f"{data.get('gpu_vram_gb', 0.0)} GB", # FIXED: MERGED SAFELY HERE
                            "GPU Load %": gpu_load,
    ...
    if rows:
        ...
        # FIXED: Optimized emoji matching rules safely
        ...
        if target_pc:
            ...
            st.dataframe(pd.DataFrame(hw.get('peripherals', [])), use_container_width=True, hide_index=True)
    else:
...
def generate_inventory_csv(target_date_str):
    ...
    try:
        ...
        for host, info in devices.items():
            ...
            storage_str = " | ".join([f"{s.get('Model','')} ({s.get('Type','')}) Letter: {s.get('DriveLetter','')} Size: {s.get('Size','')} Used: {s.get('Used_Pct',0)}%" for s in storage_list])
            ...
            export_list.append({
                ...
                "GPU VRAM": f"{info.get('gpu_vram_gb', 0.0)} GB",
