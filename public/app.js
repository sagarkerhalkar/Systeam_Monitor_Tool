...
async function renderHistory(){ if(window.__rf3Ready && typeof window.rf3LoadHistoryPublic==='function'){ return window.rf3LoadHistoryPublic(); } }
...
(function(){
  ...
  function applyExactBranding(){
    try{
      ...
      if(search){ search.classList.remove('brand-hidden-search'); search.classList.add('one-global-search'); }
...
(function(){
  const STAB2_METRICS = ['cpu_ram_combined_percent','cpu_ram_peak_percent','cpu_gpu_temp_combined_c','cpu_gpu_temp_peak_c','ram_gpu_usage_combined_percent','ram_gpu_usage_peak_percent'];
  function stab2IsEditing(){
    const el = document.activeElement; if(!el) return false;
    const tag = (el.tagName || '').toLowerCase();
    if(el.isContentEditable || tag === 'textarea') return true;
    if(tag === 'input'){ const type=(el.getAttribute('type')||'text').toLowerCase(); return !['button','submit','checkbox','radio','range','file'].includes(type); }
    return false;
  }
  function stab2Label(m){ try{return machineLabel(m);}catch(e){try{return host(m)+' - '+(m.primary_ip||'No IP');}catch(_){return (m&&(m.hostname||m.machine_id))||'';}} }
  function stab2Find(text){
    text = String(text||'').trim().toLowerCase(); if(!text) return null;
    const list = state.machines || [];
    return list.find(m => String(m.machine_id||'').toLowerCase()===text || stab2Label(m).toLowerCase()===text) ||
           list.find(m => stab2Label(m).toLowerCase().includes(text) || String(m.hostname||'').toLowerCase().includes(text) || String(m.primary_ip||'').toLowerCase().includes(text)) || null;
  }
  function stab2UpdateDatalist(id){ const input=document.getElementById(id+'_search'), dl=document.getElementById(id+'_list'); if(!input||!dl)return; dl.innerHTML=(state.machines||[]).map(m=>'<option value="'+esc(stab2Label(m))+'"></option>').join(''); }
  function stab2AttachSearch(id){
    const sel=document.getElementById(id); if(!sel||sel.dataset.stab2Search==='1') return; sel.dataset.stab2Search='1';
    const wrap=document.createElement('div'); wrap.className='stab2-machine-search-wrap'; wrap.innerHTML='<input id="'+id+'_search" class="stab2-machine-search" list="'+id+'_list" placeholder="Type client name / IP to search..." autocomplete="off"><datalist id="'+id+'_list"></datalist>';
    sel.parentNode.insertBefore(wrap, sel); const input=document.getElementById(id+'_search');
    input.addEventListener('input', function(){ const m=stab2Find(input.value); if(m&&m.machine_id){ sel.value=m.machine_id; localStorage.setItem('sagar_'+id,m.machine_id); if(!['historyMachine','changeMachine','messageMachine'].includes(id)){ state.selected=m.machine_id; localStorage.setItem('sagar_selected_machine',m.machine_id); } } });
    input.addEventListener('change', function(){ const m=stab2Find(input.value); if(m&&m.machine_id){ sel.value=m.machine_id; try{onMachineSelect(id);}catch(e){sel.dispatchEvent(new Event('change',{bubbles:true}));} } });
    sel.addEventListener('change', function(){ const m=(state.machines||[]).find(x=>String(x.machine_id||'')===String(sel.value||'')); input.value=m?stab2Label(m):''; });
    stab2UpdateDatalist(id); const m=(state.machines||[]).find(x=>String(x.machine_id||'')===String(sel.value||'')); if(m) input.value=stab2Label(m);
  }
  function stab2InstallSearches(){ ['dashboardMachine','machineSelect','softwareMachine','usbMachine','historyMachine','messageMachine','changeMachine'].forEach(stab2AttachSearch); ['dashboardMachine','machineSelect','softwareMachine','usbMachine','historyMachine','messageMachine','changeMachine'].forEach(stab2UpdateDatalist); }

  const oldSelectedMachine = (typeof selectedMachine === 'function') ? selectedMachine : null;
  selectedMachine = function(selectId){
    const id = selectId || 'machineSelect'; const sel=document.getElementById(id); const machines=state.machines||[]; let value=sel&&sel.value?String(sel.value):'';
    if(!value && id!=='historyMachine' && id!=='changeMachine') value=String(state.selected||localStorage.getItem('sagar_selected_machine')||'');
    let m=machines.find(x=>String(x.machine_id||'')===value)||null;
    if(!m && sel && sel.selectedIndex>=0) m=stab2Find(sel.options[sel.selectedIndex]?.textContent||'');
    if(!m && oldSelectedMachine){ try{m=oldSelectedMachine.apply(this,arguments);}catch(e){} }
    if(!m && machines.length) m=machines[0];
    if(m&&m.machine_id&&id!=='historyMachine'&&id!=='changeMachine'){ state.selected=m.machine_id; localStorage.setItem('sagar_selected_machine',m.machine_id); if(sel&&Array.from(sel.options||[]).some(o=>o.value===m.machine_id)) sel.value=m.machine_id; }
    return m;
  }; window.selectedMachine=selectedMachine;

  const oldHydrateSelectors=(typeof hydrateSelectors==='function')?hydrateSelectors:null;
  if(oldHydrateSelectors&&!window.__stab2HydrateWrapped){ window.__stab2HydrateWrapped=true; hydrateSelectors=function(){ const active=document.activeElement, activeId=active&&active.id; const keep={}; (selectorIds||[]).forEach(id=>{const el=document.getElementById(id); if(el) keep[id]=el.value;}); oldHydrateSelectors.apply(this,arguments); Object.keys(keep).forEach(id=>{const el=document.getElementById(id); if(el&&keep[id]&&Array.from(el.options||[]).some(o=>o.value===keep[id])) el.value=keep[id];}); stab2InstallSearches(); if(activeId&&document.getElementById(activeId)) document.getElementById(activeId).focus(); }; window.hydrateSelectors=hydrateSelectors; }

  const oldOnMachineSelect=(typeof onMachineSelect==='function')?onMachineSelect:null;
  if(oldOnMachineSelect&&!window.__stab2OnMachineWrapped){ window.__stab2OnMachineWrapped=true; onMachineSelect=function(id){ const v=document.getElementById(id)?.value||''; localStorage.setItem('sagar_'+id,v); if(v&&!['historyMachine','changeMachine','messageMachine'].includes(id)){state.selected=v; localStorage.setItem('sagar_selected_machine',v);} if(stab2IsEditing()) return; return oldOnMachineSelect.apply(this,arguments); }; window.onMachineSelect=onMachineSelect; }

  const oldRefresh=(typeof refresh==='function')?refresh:null;
  if(oldRefresh&&!window.__stab2RefreshWrapped){ window.__stab2RefreshWrapped=true; refresh=async function(manual=false){ if(!manual&&stab2IsEditing()){ try{ const data=await api('/api/overview'); state.overview=data; state.machines=data.machines||[]; state.lastRefresh=new Date(); const lr=document.getElementById('lastRefreshText'); if(lr) lr.textContent='Background updated '+state.lastRefresh.toLocaleTimeString(); state.pendingUpdate=true; try{showBanner(true);}catch(e){} stab2InstallSearches(); }catch(e){console.error(e);} return; } const out=await oldRefresh.apply(this,arguments); stab2InstallSearches(); return out; }; window.refresh=refresh; }

  const oldApiGet=(typeof apiGet==='function')?apiGet:null;
  if(oldApiGet&&!window.__stab2ApiGetWrapped){ window.__stab2ApiGetWrapped=true; apiGet=function(url){ const s=String(url||''); if(s.includes('/api/hardware-inventory-save?')||s.includes('/api/software-inventory-save?')){ try{ const u=new URL(s, location.origin); const payload=u.searchParams.get('payload')||'{}'; const obj=JSON.parse(payload); return api(u.pathname,{method:'POST',body:JSON.stringify(obj||{})}); }catch(e){ console.error('POST save conversion failed',e); } } return oldApiGet.apply(this,arguments); }; window.apiGet=apiGet; }
  const oldApiPost=(typeof apiPost==='function')?apiPost:null;
  if(oldApiPost&&!window.__stab2ApiPostWrapped){ window.__stab2ApiPostWrapped=true; apiPost=function(url,obj){ if(String(url).includes('/api/hardware-inventory-save')||String(url).includes('/api/software-inventory-save')) return api(url.split('?')[0],{method:'POST',body:JSON.stringify(obj||{})}); return oldApiPost.apply(this,arguments); }; window.apiPost=apiPost; }

  try{ if(Array.isArray(metrics)) STAB2_METRICS.forEach(m=>{if(!metrics.includes(m)) metrics.push(m);}); }catch(e){}
  const oldRenderRules=(typeof renderRules==='function')?renderRules:null;
  if(oldRenderRules&&!window.__stab2RulesWrapped){ window.__stab2RulesWrapped=true; renderRules=function(){ const out=oldRenderRules.apply(this,arguments); const sel=document.getElementById('ruleMetric'); if(sel){ const existing=Array.from(sel.options).map(o=>o.value||o.textContent); const labels={cpu_ram_combined_percent:'CPU + RAM both usage %',cpu_ram_peak_percent:'CPU/RAM peak usage %',cpu_gpu_temp_combined_c:'CPU + GPU both temp C',cpu_gpu_temp_peak_c:'CPU/GPU peak temp C',ram_gpu_usage_combined_percent:'RAM + GPU both usage %',ram_gpu_usage_peak_percent:'RAM/GPU peak usage %'}; STAB2_METRICS.forEach(m=>{if(!existing.includes(m)){const opt=document.createElement('option'); opt.value=m; opt.textContent=m+' - '+labels[m]; sel.appendChild(opt);}}); } return out; }; window.renderRules=renderRules; }

  function stab2InstallStyle(){ if(document.getElementById('stab2GlobalStyle')) return; const s=document.createElement('style'); s.id='stab2GlobalStyle'; s.textContent=`.stab2-machine-search-wrap{display:inline-flex;min-width:220px;margin:0 8px 8px 0;vertical-align:middle}.stab2-machine-search{width:min(360px,55vw);border:1px solid rgba(148,163,184,.45);border-radius:12px;padding:9px 12px;font-weight:750;background:#fff;color:#0f172a}#dayHistoryFastUsageV2{margin:14px 0 18px;border-radius:22px;padding:14px;border:1px solid rgba(94,234,212,.22);background:linear-gradient(135deg,rgba(15,23,42,.95),rgba(30,41,59,.86));box-shadow:0 20px 50px rgba(15,23,42,.28);color:#e5f9ff}#dayHistoryFastUsageV2 h2{margin:0;color:#fff}#dayHistoryFastUsageV2 .cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin:12px 0}#dayHistoryFastUsageV2 .card{border-radius:16px;padding:12px;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.12)}#dayHistoryFastUsageV2 .card span{display:block;text-transform:uppercase;letter-spacing:.08em;font-size:11px;color:#93c5fd;font-weight:900}#dayHistoryFastUsageV2 .card strong{display:block;margin-top:6px;font-size:22px;color:#fff}#dayHistoryFastUsageV2 .controls{display:flex;gap:8px;flex-wrap:wrap;margin:10px 0}#dayHistoryFastUsageV2 select,#dayHistoryFastUsageV2 input,#dayHistoryFastUsageV2 button{border-radius:12px;padding:9px 10px;font-weight:850;border:1px solid rgba(255,255,255,.16)}#dayHistoryFastUsageV2 select,#dayHistoryFastUsageV2 input{background:#0f172a;color:#e5f9ff}#dayHistoryFastUsageV2 button{background:#5eead4;color:#042f2e;cursor:pointer}#dayHistoryFastUsageV2 .tablewrap{max-height:330px;overflow:auto;border-radius:16px;border:1px solid rgba(255,255,255,.12)}#dayHistoryFastUsageV2 table{width:100%;border-collapse:collapse;min-width:980px;font-size:13px}#dayHistoryFastUsageV2 th{position:sticky;top:0;background:rgba(15,23,42,.96);color:#bfdbfe;text-align:left;padding:10px;text-transform:uppercase;letter-spacing:.06em;font-size:11px}#dayHistoryFastUsageV2 td{padding:10px;border-top:1px solid rgba(255,255,255,.08);color:#e2e8f0;font-weight:650}@media(max-width:760px){.stab2-machine-search-wrap,.stab2-machine-search{width:100%;min-width:0}.stab2-machine-search{width:100%}}`; document.head.appendChild(s); }
  function gb(v){v=Number(v||0); return v>=1024?(v/1024).toFixed(2).replace(/\.00$/,'')+' TB':v.toFixed(2).replace(/\.00$/,'')+' GB';}
  function mbps(v){return Number(v||0).toFixed(2).replace(/\.00$/,'')+' Mbps';}
  let fastRows=[];
  function historyFastPanel(){ const page=document.getElementById('page-history'); if(!page)return null; stab2InstallStyle(); ['historyDataUsageTotalsPanel','dayHistoryUsage3DV2','dayHistoryUsageFastPanelV3','dayHistoryFastUsageV1'].forEach(id=>{const el=document.getElementById(id); if(el)el.remove();}); let panel=document.getElementById('dayHistoryFastUsageV2'); if(panel)return panel; const html=`<section id="dayHistoryFastUsageV2"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap"><div><h2>Fast Upload / Download History</h2><p>Total internet data by custom date range and machine. This does not reload the slow change-history list.</p></div><button id="hfu2Csv" type="button">Download CSV</button></div><div class="controls"><select id="hfu2Machine"><option value="">All machines</option></select><select id="hfu2Days"><option value="7">Last 7 days</option><option value="30" selected>Last 30 days</option><option value="90">Last 90 days</option><option value="365">Last 365 days</option></select><input id="hfu2From" type="date"><input id="hfu2To" type="date"><button id="hfu2Load" type="button">Load Fast Usage</button></div><div class="cards" id="hfu2Cards"><div class="card"><span>Status</span><strong>Ready</strong></div></div><p id="hfu2Note">Select machine/date and click Load Fast Usage.</p><div class="tablewrap"><table><thead><tr><th>Machine</th><th>LAN / Public IP</th><th>ISP</th><th>Days</th><th>Heartbeats</th><th>Total Download</th><th>Total Upload</th><th>Total Data</th><th>Max Down</th><th>Max Up</th><th>Last Seen</th></tr></thead><tbody id="hfu2Body"><tr><td colspan="11">Click Load Fast Usage.</td></tr></tbody></table></div></section>`; const anchor=page.querySelector('.history-hero,.history-fast-shell,.history-shell,.panel'); if(anchor)anchor.insertAdjacentHTML('afterend',html); else page.insertAdjacentHTML('afterbegin',html); fillHistoryMachineSelect(); document.getElementById('hfu2Load')?.addEventListener('click',()=>loadFastHistory(true)); ['hfu2Days','hfu2Machine','hfu2From','hfu2To'].forEach(id=>document.getElementById(id)?.addEventListener('change',()=>loadFastHistory(true))); document.getElementById('hfu2Csv')?.addEventListener('click',downloadFastHistoryCsv); return document.getElementById('dayHistoryFastUsageV2'); }
  function fillHistoryMachineSelect(){ const sel=document.getElementById('hfu2Machine'); if(!sel)return; const keep=sel.value||''; sel.innerHTML='<option value="">All machines</option>'+(state.machines||[]).map(m=>`<option value="${esc(m.machine_id||'')}">${esc(stab2Label(m))}</option>`).join(''); if(keep)sel.value=keep; }
  function fastQuery(){ const p=new URLSearchParams(); p.set('days',document.getElementById('hfu2Days')?.value||'30'); const mid=document.getElementById('hfu2Machine')?.value||'', df=document.getElementById('hfu2From')?.value||'', dt=document.getElementById('hfu2To')?.value||''; if(mid)p.set('machine_id',mid); if(df)p.set('date_from',df); if(dt)p.set('date_to',dt); return p.toString(); }
  let fastLoading=false;
  async function loadFastHistory(){ if(state.page!=='history'||fastLoading)return; historyFastPanel(); fastLoading=true; const body=document.getElementById('hfu2Body'), note=document.getElementById('hfu2Note'); if(body)body.innerHTML='<tr><td colspan="11">Loading fast usage...</td></tr>'; if(note)note.textContent='Loading /api/history-usage-fast only...'; try{ const res=await api('/api/history-usage-fast?'+fastQuery()); if(!res.ok)throw new Error(res.error||'Fast history API failed'); fastRows=res.machines||[]; renderFastHistory(res); }catch(e){ if(body)body.innerHTML='<tr><td colspan="11">Fast usage failed. Try Last 7 days or select one machine.</td></tr>'; if(note)note.textContent='Error: '+(e.message||e); }finally{ fastLoading=false; } }
  function renderFastHistory(res){ const rows=res.machines||[]; const totalDown=rows.reduce((a,x)=>a+Number(x.download_gb||0),0), totalUp=rows.reduce((a,x)=>a+Number(x.upload_gb||0),0); const cards=document.getElementById('hfu2Cards'); if(cards)cards.innerHTML=`<div class="card"><span>Total Download</span><strong>${gb(totalDown)}</strong><small>${esc(res.date_from||'')} to ${esc(res.date_to||'')}</small></div><div class="card"><span>Total Upload</span><strong>${gb(totalUp)}</strong><small>Selected range</small></div><div class="card"><span>Total Data</span><strong>${gb(totalDown+totalUp)}</strong><small>Download + Upload</small></div><div class="card"><span>Machines</span><strong>${rows.length}</strong><small>With usage</small></div><div class="card"><span>Days</span><strong>${(res.daily||[]).length}</strong><small>Stored records</small></div>`; const body=document.getElementById('hfu2Body'); if(body)body.innerHTML=rows.map(x=>{const ip=[x.primary_ip,x.public_ip].filter(Boolean).join(' / '); return `<tr><td><strong>${esc(x.hostname||x.machine_id||'')}</strong></td><td>${esc(ip)}</td><td>${esc(x.isp_name||'')}</td><td>${esc(x.days||0)}</td><td>${esc(x.heartbeat_count||0)}</td><td><strong>${gb(x.download_gb)}</strong></td><td><strong>${gb(x.upload_gb)}</strong></td><td><strong>${gb(x.total_gb)}</strong></td><td>${mbps(x.max_down_mbps)}</td><td>${mbps(x.max_up_mbps)}</td><td>${esc(x.last_seen||'')}</td></tr>`;}).join('')||'<tr><td colspan="11">No data for this selection.</td></tr>'; const note=document.getElementById('hfu2Note'); if(note)note.textContent='Loaded using fast SQL endpoint. This will not disturb writing or slow page list.'; }
  function downloadFastHistoryCsv(){ if(!fastRows.length){alert('Load Fast Usage first.');return;} const header=['Machine','Machine ID','LAN IP','Public IP','ISP','Days','Heartbeats','Download GB','Upload GB','Total GB','Max Down Mbps','Max Up Mbps','Last Seen']; const rows=[header].concat(fastRows.map(x=>[x.hostname,x.machine_id,x.primary_ip,x.public_ip,x.isp_name,x.days,x.heartbeat_count,x.download_gb,x.upload_gb,x.total_gb,x.max_down_mbps,x.max_up_mbps,x.last_seen])); const csv=rows.map(r=>r.map(v=>'"'+String(v??'').replace(/"/g,'""')+'"').join(',')).join('\n'); const blob=new Blob([csv],{type:'text/csv;charset=utf-8'}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='day_history_fast_usage.csv'; document.body.appendChild(a); a.click(); setTimeout(()=>{URL.revokeObjectURL(url); a.remove();},500); }

  const oldSwitch=window.switchPage||(typeof switchPage!=='undefined'?switchPage:null);
  if(oldSwitch&&!window.__stab2SwitchWrapped){ window.__stab2SwitchWrapped=true; window.switchPage=function(page){ oldSwitch.apply(this,arguments); stab2InstallSearches(); if(page==='history')setTimeout(()=>{historyFastPanel(); fillHistoryMachineSelect();},200); }; try{switchPage=window.switchPage;}catch(e){} }
  setInterval(function(){ stab2InstallSearches(); if(state.page==='history'){historyFastPanel(); fillHistoryMachineSelect();}},3000);
  setTimeout(function(){stab2InstallStyle(); stab2InstallSearches(); if(state.page==='history')historyFastPanel();},1000);
})();
/* STABILITY_AUDIT_TESTED_V2_END */

/* REAL_FIX_V3_START */
(function(){
  const RF3_METRICS = ['cpu_ram_combined_percent','cpu_ram_peak_percent','cpu_gpu_temp_combined_c','cpu_gpu_temp_peak_c','ram_gpu_usage_combined_percent','ram_gpu_usage_peak_percent'];
  try{ if(Array.isArray(metrics)) RF3_METRICS.forEach(m=>{ if(!metrics.includes(m)) metrics.push(m); }); }catch(e){}

  function rf3Editing(){ const a=document.activeElement; return !!(a && (a.matches('input,textarea,select,[contenteditable="true"]') || a.closest('.rf3-no-refresh'))); }
  function rf3Esc(v){ try{return esc(v);}catch(e){return String(v==null?'':v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));} }
  function rf3Host(m){ try{return host(m);}catch(e){return (m&&(m.hostname||m.host||m.machine_id))||'';} }
  function rf3Label(m){ return rf3Host(m)+' - '+(m.primary_ip||((m.all_ips||[])[0]||'No IP')); }
  function rf3Gb(v){ v=Number(v||0); if(!Number.isFinite(v))v=0; return v>=1024?(v/1024).toFixed(2).replace(/\.00$/,'')+' TB':v.toFixed(2).replace(/\.00$/,'')+' GB'; }
  function rf3Mbps(v){ v=Number(v||0); if(!Number.isFinite(v))v=0; return v.toFixed(2).replace(/\.00$/,'')+' Mbps'; }

  window.selectedMachine = function(id){
    const machines=state.machines||[];
    const sel=document.getElementById(id||'machineSelect') || document.getElementById('machineSelect');
    let val=(sel&&sel.value)||state.selected||localStorage.getItem('sagar_selected_machine')||'';
    let m=machines.find(x=>String(x.machine_id||'')===String(val))||null;
    if(!m && sel && sel.selectedIndex>=0){ const txt=(sel.options[sel.selectedIndex]?.textContent||'').trim().toLowerCase(); m=machines.find(x=>txt.includes(rf3Host(x).toLowerCase()) && txt.includes(String(x.primary_ip||'').toLowerCase()))||null; }
    if(!m && machines.length) m=machines[0];
    if(m){ state.selected=String(m.machine_id||''); localStorage.setItem('sagar_selected_machine',state.selected); if(sel && Array.from(sel.options).some(o=>o.value===state.selected)) sel.value=state.selected; }
    return m;
  };
  try{ selectedMachine=window.selectedMachine; }catch(e){}

  const oldRefresh = (typeof refresh==='function') ? refresh : null;
  if(oldRefresh && !window.__rf3RefreshWrapped){
    window.__rf3RefreshWrapped=true;
    window.refresh = async function(manual=false){
      if(!manual && rf3Editing()){
        try{ const data=await api('/api/overview'); state.overview=data; state.machines=data.machines||[]; state.lastRefresh=new Date(); state.pendingUpdate=true; showBanner(true); if($('#lastRefreshText')) $('#lastRefreshText').textContent='Background updated '+state.lastRefresh.toLocaleTimeString(); rf3EnsureSelectorSearches(); }
        catch(e){ console.error(e); }
        return;
      }
      const r = await oldRefresh.apply(this, arguments);
      rf3EnsureSelectorSearches();
      return r;
    };
    try{ refresh=window.refresh; }catch(e){}
  }

  function rf3FilterSelector(id){
    const sel=document.getElementById(id); const input=document.getElementById('rf3Search_'+id); if(!sel||!input) return;
    const q=(input.value||'').trim().toLowerCase(); const keep=sel.value||'';
    const allowAll=(id==='historyMachine'||id==='changeMachine'||id==='messageMachine');
    let html=allowAll?'<option value="">All machines</option>':'';
    (state.machines||[]).filter(m=>!q || rf3Label(m).toLowerCase().includes(q) || String(m.machine_id||'').toLowerCase().includes(q) || String(m.public_ip||'').toLowerCase().includes(q)).forEach(m=>{ html+='<option value="'+rf3Esc(m.machine_id||'')+'">'+rf3Esc(rf3Label(m))+'</option>'; });
    sel.innerHTML=html || '<option value="">No match</option>';
    if(keep && Array.from(sel.options).some(o=>o.value===keep)) sel.value=keep;
  }

  function rf3EnsureSelectorSearches(){
    const ids=(typeof selectorIds!=='undefined'&&Array.isArray(selectorIds))?selectorIds:['dashboardMachine','machineSelect','softwareMachine','usbMachine','historyMachine','messageMachine','changeMachine'];
    ids.forEach(id=>{
      const sel=document.getElementById(id); if(!sel || document.getElementById('rf3Search_'+id)) return;
      const inp=document.createElement('input'); inp.id='rf3Search_'+id; inp.className='rf3-machine-search rf3-no-refresh'; inp.placeholder='Type client name / IP to search'; inp.style.cssText='width:100%;max-width:360px;margin:6px 0 8px;padding:9px 11px;border-radius:12px;border:1px solid rgba(148,163,184,.35);';
      sel.parentNode.insertBefore(inp, sel);
      inp.addEventListener('input',()=>rf3FilterSelector(id));
      sel.addEventListener('focus',()=>rf3FilterSelector(id));
    });
  }

  const oldHydrate = (typeof hydrateSelectors==='function') ? hydrateSelectors : null;
  if(oldHydrate && !window.__rf3HydrateWrapped){ window.__rf3HydrateWrapped=true; hydrateSelectors=function(){ oldHydrate.apply(this,arguments); rf3EnsureSelectorSearches(); }; window.hydrateSelectors=hydrateSelectors; }

  const oldRenderAll=(typeof renderAll==='function')?renderAll:null;
  if(oldRenderAll && !window.__rf3RenderAllWrapped){ window.__rf3RenderAllWrapped=true; renderAll=function(){ if(rf3Editing()){ state.pendingUpdate=true; showBanner(true); return; } const r=oldRenderAll.apply(this,arguments); rf3EnsureSelectorSearches(); return r; }; window.renderAll=renderAll; }

  const oldApiGet=(typeof apiGet==='function')?apiGet:null;
  if(oldApiGet&&!window.__rf3ApiGetWrapped){ window.__rf3ApiGetWrapped=true; apiGet=function(url){ const s=String(url||''); if(s.includes('/api/hardware-inventory-save?')||s.includes('/api/software-inventory-save?')){ const u=new URL(s, location.origin); const payload=u.searchParams.get('payload')||'{}'; let obj={}; try{ obj=JSON.parse(payload); }catch(e){} return api(u.pathname,{method:'POST',body:JSON.stringify(obj||{})}); } return oldApiGet.apply(this,arguments); }; window.apiGet=apiGet; }
  const oldApiPost=(typeof apiPost==='function')?apiPost:null;
  if(oldApiPost&&!window.__rf3ApiPostWrapped){ window.__rf3ApiPostWrapped=true; apiPost=function(url,obj){ if(String(url).includes('/api/hardware-inventory-save')||String(url).includes('/api/software-inventory-save')) return api(String(url).split('?')[0],{method:'POST',body:JSON.stringify(obj||{})}); return oldApiPost.apply(this,arguments); }; window.apiPost=apiPost; }

  let rf3Rows=[];
  function rf3Panel(){
    const page=document.getElementById('page-history'); if(!page) return null;
    let p=document.getElementById('rf3HistoryPanel'); if(p) return p;
    const html='<section class="panel rf3-no-refresh" id="rf3HistoryPanel"><div class="panel-head"><div><h2>Fast Upload / Download History</h2><p class="muted">Custom date + machine-wise internet data total. This does not load slow heartbeat samples.</p></div><button class="btn small download-only" id="rf3CsvBtn">Download CSV</button></div><div class="grid cols-4"><label>Machine<select id="rf3HistMachine"><option value="">All machines</option></select></label><label>From<input id="rf3HistFrom" type="date"></label><label>To<input id="rf3HistTo" type="date"></label><label>Days<select id="rf3HistDays"><option value="7">7</option><option value="30" selected>30</option><option value="90">90</option><option value="365">365</option></select></label></div><div style="margin:10px 0"><button class="btn" id="rf3LoadBtn">Load Fast Usage</button></div><div id="rf3Cards" class="kpi-grid"></div><div class="table-wrap"><table><thead><tr><th>Machine</th><th>IP</th><th>ISP</th><th>Days</th><th>Heartbeats</th><th>Download</th><th>Upload</th><th>Total</th><th>Max Down</th><th>Max Up</th><th>Last Seen</th></tr></thead><tbody id="rf3HistBody"><tr><td colspan="11" class="empty">Click Load Fast Usage.</td></tr></tbody></table></div><p class="muted" id="rf3Note">Ready.</p></section>';
    const anchor=page.querySelector('.history-hero,.panel,.history-fast-shell,.history-shell'); if(anchor) anchor.insertAdjacentHTML('afterend',html); else page.insertAdjacentHTML('afterbegin',html);
    p=document.getElementById('rf3HistoryPanel');
    const sel=document.getElementById('rf3HistMachine'); if(sel){ sel.innerHTML='<option value="">All machines</option>'+(state.machines||[]).map(m=>'<option value="'+rf3Esc(m.machine_id||'')+'">'+rf3Esc(rf3Label(m))+'</option>').join(''); }
    document.getElementById('rf3LoadBtn')?.addEventListener('click',()=>rf3LoadHistory(true));
    document.getElementById('rf3CsvBtn')?.addEventListener('click',rf3Csv);
    ['rf3HistMachine','rf3HistFrom','rf3HistTo','rf3HistDays'].forEach(id=>document.getElementById(id)?.addEventListener('change',()=>rf3LoadHistory(true)));
    return p;
  }
  async function rf3LoadHistory(force){
    if(state.page!=='history') return; rf3Panel();
    const params=new URLSearchParams(); params.set('days',document.getElementById('rf3HistDays')?.value||'30'); const mid=document.getElementById('rf3HistMachine')?.value||'', df=document.getElementById('rf3HistFrom')?.value||'', dt=document.getElementById('rf3HistTo')?.value||''; if(mid)params.set('machine_id',mid); if(df)params.set('date_from',df); if(dt)params.set('date_to',dt);
    const body=document.getElementById('rf3HistBody'), note=document.getElementById('rf3Note'); if(body) body.innerHTML='<tr><td colspan="11" class="empty">Loading fast usage...</td></tr>'; if(note) note.textContent='Loading /api/history-usage-fast...';
    try{ const d=await api('/api/history-usage-fast?'+params.toString()); if(!d.ok) throw new Error(d.error||'fast history failed'); rf3Rows=d.machines||[]; const td=rf3Rows.reduce((a,x)=>a+Number(x.download_gb||0),0), tu=rf3Rows.reduce((a,x)=>a+Number(x.upload_gb||0),0); const cards=document.getElementById('rf3Cards'); if(cards)cards.innerHTML='<div class="kpi"><span>Total Download</span><strong>'+rf3Gb(td)+'</strong></div><div class="kpi"><span>Total Upload</span><strong>'+rf3Gb(tu)+'</strong></div><div class="kpi"><span>Total Data</span><strong>'+rf3Gb(td+tu)+'</strong></div><div class="kpi"><span>Machines</span><strong>'+rf3Rows.length+'</strong></div>'; if(body)body.innerHTML=rf3Rows.map(x=>'<tr><td><strong>'+rf3Esc(x.hostname||x.machine_id||'')+'</strong></td><td>'+rf3Esc([x.primary_ip,x.public_ip].filter(Boolean).join(' / '))+'</td><td>'+rf3Esc(x.isp_name||'')+'</td><td>'+rf3Esc(x.days||0)+'</td><td>'+rf3Esc(x.heartbeat_count||0)+'</td><td><strong>'+rf3Gb(x.download_gb)+'</strong></td><td><strong>'+rf3Gb(x.upload_gb)+'</strong></td><td><strong>'+rf3Gb(x.total_gb)+'</strong></td><td>'+rf3Mbps(x.max_down_mbps)+'</td><td>'+rf3Mbps(x.max_up_mbps)+'</td><td>'+rf3Esc(x.last_seen||'')+'</td></tr>').join('')||'<tr><td colspan="11" class="empty">No usage for selection.</td></tr>'; if(note) note.textContent='Loaded fast history: '+rf3Esc(d.date_from||'')+' to '+rf3Esc(d.date_to||''); }
    catch(e){ if(body)body.innerHTML='<tr><td colspan="11" class="empty">Fast usage failed: '+rf3Esc(e.message||e)+'</td></tr>'; if(note)note.textContent='Fast usage failed.'; }
  }
  function rf3Csv(){ if(!rf3Rows.length){alert('Load Fast Usage first.');return;} const header=['Machine','Machine ID','LAN IP','Public IP','ISP','Days','Heartbeats','Download GB','Upload GB','Total GB','Max Down Mbps','Max Up Mbps','Last Seen']; const rows=[header].concat(rf3Rows.map(x=>[x.hostname,x.machine_id,x.primary_ip,x.public_ip,x.isp_name,x.days,x.heartbeat_count,x.download_gb,x.upload_gb,x.total_gb,x.max_down_mbps,x.max_up_mbps,x.last_seen])); const csv=rows.map(r=>r.map(v=>'"'+String(v??'').replace(/"/g,'""')+'"').join(',')).join('\n'); const blob=new Blob([csv],{type:'text/csv;charset=utf-8'}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='day_history_fast_usage.csv'; document.body.appendChild(a); a.click(); setTimeout(()=>{URL.revokeObjectURL(url);a.remove();},500); }

  window.renderHistory = async function(){ rf3Panel(); await rf3LoadHistory(false); const daily=$('#historyDailyTable tbody'), mt=$('#historyMachineTable tbody'), st=$('#historySampleTable tbody'); if(daily)daily.innerHTML='<tr><td colspan="11" class="empty">Use Fast Upload / Download History panel above.</td></tr>'; if(mt)mt.innerHTML='<tr><td colspan="15" class="empty">Use Fast Upload / Download History panel above.</td></tr>'; if(st)st.innerHTML='<tr><td colspan="14" class="empty">Samples disabled to keep browser fast.</td></tr>'; };
  try{ renderHistory=window.renderHistory; }catch(e){}
...
(function(){
  const FINAL_V4_METRICS = [
    'cpu_ram_combined_percent','cpu_ram_peak_percent',
    'cpu_gpu_temp_combined_c','cpu_gpu_temp_peak_c',
    'ram_gpu_usage_combined_percent','ram_gpu_usage_peak_percent'
  ];
  const PAGE_SELECT = {dashboard:'dashboardMachine',machine360:'machineSelect',software:'softwareMachine',usb:'usbMachine',history:'historyMachine',messages:'messageMachine',changes:'changeMachine'};
  function q(s,r){return (r||document).querySelector(s)}
  function qa(s,r){return Array.from((r||document).querySelectorAll(s))}
  function txt(x){return String(x==null?'':x).toLowerCase()}
  function activePage(){return q('.page.active')||q('#page-'+(state&&state.page||''));}
  function machineText(m){return txt([host(m),m.hostname,m.machine_id,m.primary_ip,m.public_ip,m.os,(m.gpu_names||[]).join(' ')].join(' '));}
  function currentSearch(){return q('#globalSearch')?.value || state.query || '';}
  function ensureOneSearch(){
    const gs=q('#globalSearch'); if(!gs) return;
    gs.classList.remove('brand-hidden-search'); gs.classList.add('one-global-search');
    gs.style.display='inline-flex'; gs.placeholder='One Search: client, IP, asset, software, table text...';
    qa('input').forEach(function(inp){
      if(inp===gs) return;
      const meta=txt((inp.id||'')+' '+(inp.className||'')+' '+(inp.placeholder||''));
      if(meta.includes('search') || inp.classList.contains('eas-select-filter')){
        inp.classList.add('final-v4-hidden-search');
        inp.setAttribute('aria-hidden','true');
      }
    });
    qa('#easGlobalSearch,.eas-search-wrap,.eas-select-filter').forEach(x=>x.remove());
  }
  function filterDom(query){
    const page=activePage(); if(!page) return; const s=txt(query).trim();
    const items=qa('tbody tr,.card,.panel,.message-card,.timeline-card,.sw-card,.ai9-card,.usb-simple-device,article,.item,.net-card,.hw-card,.detail-card',page);
    items.forEach(function(el){
      if(el.closest('.modal')||el.closest('.ai9-modal')||el.closest('.sw-modal')||el.closest('form')) return;
      if(!s){el.classList.remove('final-v4-filter-hidden'); return;}
      el.classList.toggle('final-v4-filter-hidden', txt(el.innerText||el.textContent).indexOf(s)<0);
    });
  }
  function selectMachineFromSearch(query){
    const s=txt(query).trim(); if(s.length<2 || !Array.isArray(state.machines)) return false;
    const match=state.machines.find(m=>machineText(m).includes(s));
    if(!match) return false;
    const id=PAGE_SELECT[state.page] || 'machineSelect'; const sel=q('#'+id); if(!sel) return false;
    if(sel.value!==match.machine_id){
      sel.value=match.machine_id; localStorage.setItem('sagar_'+id, match.machine_id);
      try{ if(typeof onMachineSelect==='function') onMachineSelect(id); else renderAll(); }catch(e){ try{renderAll();}catch(_){} }
    }
    return true;
  }
  function syncPageSpecificSearch(query){
    const s=String(query||'');
    try{ if(window.__assetInvV9){ window.__assetInvV9.search=s; window.__assetInvV9.page=1; } }catch(e){}
    try{ if(window.__swInv){ window.__swInv.search=s; window.__swInv.page=1; } }catch(e){}
    try{ localStorage.setItem('sagar_software_search', s); localStorage.setItem('sagar_usb_search', s); }catch(e){}
  }
  function applyGlobalSearch(query, rerender){
    state.query=String(query||''); syncPageSpecificSearch(state.query);
    if(rerender){
      try{
        if(state.page==='fleet'||state.page==='dashboard') renderAll();
        else if(state.page==='hardware' && typeof renderHardware==='function') renderHardware(false);
        else if(state.page==='software' && typeof renderSoftwareInventory==='function') renderSoftwareInventory(false);
        else if(state.page==='usb' && typeof renderUsb==='function') renderUsb();
      }catch(e){console.warn('final v4 page rerender skipped', e);}
    }
    selectMachineFromSearch(state.query); setTimeout(()=>filterDom(state.query),80);
  }
  function ensureCompositeMetrics(){
    try{ if(Array.isArray(metrics)) FINAL_V4_METRICS.forEach(m=>{if(!metrics.includes(m)) metrics.push(m);}); }catch(e){}
    const sel=q('#ruleMetric'); if(sel){
      const have=new Set(qa('option',sel).map(o=>o.value||o.textContent));
      FINAL_V4_METRICS.forEach(m=>{ if(!have.has(m)){ const o=document.createElement('option'); o.value=m; o.textContent=m; sel.appendChild(o); } });
    }
  }
  function bind(){
    ensureOneSearch(); ensureCompositeMetrics();
    const gs=q('#globalSearch'); if(gs && !gs.dataset.finalV4Bound){
      gs.dataset.finalV4Bound='1';
      gs.addEventListener('input', function(){ clearTimeout(window.__finalV4SearchTimer); window.__finalV4SearchTimer=setTimeout(()=>applyGlobalSearch(this.value,true),120); }, true);
      gs.addEventListener('keydown', function(ev){ if(ev.key==='Enter'){ ev.preventDefault(); applyGlobalSearch(this.value,true); } }, true);
    }
    filterDom(currentSearch());
  }
  const oldRenderAll=(typeof renderAll==='function')?renderAll:null;
  if(oldRenderAll && !window.__finalV4RenderAll){
    window.__finalV4RenderAll=true;
    renderAll=function(){ const r=oldRenderAll.apply(this,arguments); setTimeout(bind,80); return r; };
    window.renderAll=renderAll;
  }
  const oldSwitch=(typeof switchPage==='function')?switchPage:null;
  if(oldSwitch && !window.__finalV4SwitchPage){
    window.__finalV4SwitchPage=true;
    switchPage=function(page){ const r=oldSwitch.apply(this,arguments); setTimeout(()=>applyGlobalSearch(currentSearch(),false),120); return r; };
    window.switchPage=switchPage;
  }
  const oldRules=(typeof renderRules==='function')?renderRules:null;
  if(oldRules && !window.__finalV4RenderRules){
    window.__finalV4RenderRules=true;
    renderRules=function(settings){ const r=oldRules.apply(this,arguments); ensureCompositeMetrics(); return r; };
    window.renderRules=renderRules;
  }
  if(!document.getElementById('finalV4OneSearchStyle')){
    const st=document.createElement('style'); st.id='finalV4OneSearchStyle'; st.textContent=`
      #globalSearch.one-global-search{display:inline-flex!important;visibility:visible!important;opacity:1!important;width:420px;max-width:46vw;height:46px;border:1px solid rgba(37,99,235,.22);border-radius:16px;background:#fff;padding:0 16px;font-weight:850;color:#0f172a;box-shadow:0 10px 24px rgba(15,33,60,.08)}
      .final-v4-hidden-search{display:none!important}.final-v4-filter-hidden{display:none!important}
      @media(max-width:900px){#globalSearch.one-global-search{width:100%;max-width:none}.top-actions{width:100%}}
    `; document.head.appendChild(st);
  }
  document.addEventListener('DOMContentLoaded',bind); document.addEventListener('click',()=>setTimeout(bind,120),true);
  document.addEventListener('input',function(ev){ if(ev.target && ev.target.id==='globalSearch') applyGlobalSearch(ev.target.value,false); },true);
  setInterval(bind,1500); bind();
  window.finalV4Evidence=function(){return {oneSearch:!!q('#globalSearch'),extraSearches:qa('#easGlobalSearch,.eas-select-filter,input.final-v4-hidden-search').length,metrics:FINAL_V4_METRICS.filter(m=>Array.isArray(metrics)&&metrics.includes(m)),page:state.page,query:currentSearch()};};
})();
