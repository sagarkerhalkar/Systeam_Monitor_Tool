    ...
    with usage_lock:
        ...
        try:
            ...
            if clean_host in devices:
                devices[clean_host]["daily_rx_bytes"] = current_rx
                devices[clean_host]["daily_tx_bytes"] = current_tx
            else:
                devices[clean_host] = {
...
# REAL LIVE SPEED TEST WORKER - FIXED NO MORE FAKE PING LOGIC
def run_speed_test_worker():
    isp_title = "Scanning WAN Interfaces..."
    try:
        ...
        # Calculate real mathematical mega-bits values
...
def monitor_internet_speed_loop():
    while True:
        ...
        # Increased to 5 minutes (300 seconds) so real tests have plenty of processing time
        time.sleep(300)
...
class MetricReceiver(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            print(f"[RECEIVER POST ERROR] Broken packet payload: {e}")
