...
alert_state = {
    "download_under_100_triggered": False,
    "download_under_50_triggered": False,
    "last_download_5min_alert": 0,
    
    "upload_under_100_triggered": False,
    "upload_under_50_triggered": False,
    "last_upload_5min_alert": 0,
...
def run_speed_test_worker():
    ...
    try:
        ...
        # ==================== DOWNLOAD THRESHOLDS ====================
        if download_speed < 50.0:
            alert_state["download_under_100_triggered"] = True
            if (current_time - alert_state["last_download_5min_alert"] >= 300):
                send_chat_notification(f"🚨 *CRITICAL DOWNLOAD DEGRADATION* 🚨\n⚠️ Download speed has dropped below 50 Mbps!\n▪️ *Current Download:* {round(download_speed, 1)} Mbps\n⏳ Repeats every 5 mins.")
                alert_state["last_download_5min_alert"] = current_time
        elif download_speed < 100.0:
            if not alert_state["download_under_100_triggered"]:
                send_chat_notification(f"⚠️ *DOWNLOAD SPEED WARNING (< 100 Mbps)* ⚠️\n▪️ *Current Download:* {round(download_speed, 1)} Mbps")
                alert_state["download_under_100_triggered"] = True
        else:
            if alert_state["download_under_100_triggered"]:
                send_chat_notification(f"✅ *DOWNLOAD SPEED RECOVERED* ✅\n▪️ *Current Download:* {round(download_speed, 1)} Mbps")
            alert_state["download_under_100_triggered"] = False

        # ==================== UPLOAD THRESHOLDS ====================
        if upload_speed < 50.0:
            alert_state["upload_under_100_triggered"] = True
            if (current_time - alert_state["last_upload_5min_alert"] >= 300):
                send_chat_notification(f"🚨 *CRITICAL UPLOAD DEGRADATION* 🚨\n⚠️ Upload speed has dropped below 50 Mbps!\n▪️ *Current Upload:* {round(upload_speed, 1)} Mbps\n⏳ Repeats every 5 mins.")
                alert_state["last_upload_5min_alert"] = current_time
        elif upload_speed < 100.0:
            if not alert_state["upload_under_100_triggered"]:
                send_chat_notification(f"⚠️ *UPLOAD SPEED WARNING (< 100 Mbps)* ⚠️\n▪️ *Current Upload:* {round(upload_speed, 1)} Mbps")
                alert_state["upload_under_100_triggered"] = True
        else:
            if alert_state["upload_under_100_triggered"]:
                send_chat_notification(f"✅ *UPLOAD SPEED RECOVERED* ✅\n▪️ *Current Upload:* {round(upload_speed, 1)} Mbps")
            alert_state["upload_under_100_triggered"] = False
...
if __name__ == "__main__":
    ...
    # 2. Fire immediate startup notification
    print("[DIAGNOSTIC] Firing initial verification string to Space channel...")
    send_chat_notification("🚨 *SERVER SUB-ENGINE READY* 🚨\nMulti-threaded listener engine and GUI Dashboard successfully compiled and loaded.")
    
    # 3. --- NEW LAUNCHER LINE: Automatically boot app.py quietly ---
    ...
    # This fires off the dashboard as an independent process so it runs silently inside the service
