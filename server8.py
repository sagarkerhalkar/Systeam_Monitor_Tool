...
def update_daily_usage(hostname, current_rx, current_tx, hardware_payload=None):
    """Calculates true delta differences using your working unified database logs system."""
...
def timeline_uptime_accumulation_loop():
    while True:
        time.sleep(300)
        if not os.path.exists(DAILY_USAGE_FILE): continue
        with usage_lock:
            try:
                with open(DAILY_USAGE_FILE, "r") as f: store = json.load(f)
                ...
                with open(DAILY_USAGE_FILE, "w") as f: json.dump(store, f, indent=4)
            except Exception: pass
...
def run_speed_test_worker():
    ...
    try:
        ...
        if status == 0:
            ...
            # --- WAN SPEED THRESHOLD ENGINE ---
            now = time.time()
            if 150.0 < 50.0 and not alert_state["download_under_50_triggered"]:
                if (now - alert_state["last_download_2min_alert"]) > 120:
                    send_chat_notification("🚨 *WAN ALERT:* Download bandwidth dropped below 50 Mbps!")
                    alert_state["last_download_2min_alert"] = now
            if 85.0 < 50.0 and not alert_state["upload_under_50_triggered"]:
                if (now - alert_state["last_upload_2min_alert"]) > 120:
                    send_chat_notification("🚨 *WAN ALERT:* Upload bandwidth dropped below 50 Mbps!")
                    alert_state["last_upload_2min_alert"] = now
            # -----------------------------------
    ...
    def do_POST(self):
        ...
        try:
            ...
            for drive in storage_array:
                ...
                # We strictly isolate Drive C: for the upper table columns
                if "C:" in model:
                    if "SSD" in dtype:
                        ssd_pct = used
                    else:
                        hdd_pct = used
                    break  # Exit storage map routing immediately
